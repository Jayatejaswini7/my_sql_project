-- create a database
create database employee_mangament_system;

use employee_mangament_system;
...
-- Q.1. How many unique employees are currently in the system?
SELECT COUNT(DISTINCT emp_ID) AS total_unique_employees
FROM Employee;

-- Q.2.Which departments have the highest number of employees?
SELECT 
...
ORDER BY 
 ...
 -- Q.3.What is the average salary per department?
SELECT 
    jd.Job_ID,
...
-- Q.4.Who are the top 5 highest-paid employees?
SELECT 
    e.emp_ID,
    e.firstname,
    e.lastname,
...
-- Q.5.What is the total salary expenditure across the company?
SELECT 
...
-- Q.1.How many different job roles exist in each department?
SELECT
    jd.name AS department_name,
    COUNT(DISTINCT jd.Job_ID) AS number_of_job_roles
FROM
    JobDepartment jd
GROUP BY
    jd.name
...
-- Q.2.What is the average salary range per department?
SELECT
    jd.jobdept AS department,
    (MIN(sb.annual) + MAX(sb.annual)) / 2 AS average_annual_salary_range
FROM
    JobDepartment jd
JOIN
    SalaryBonus sb ON jd.Job_ID = sb.Job_ID
GROUP BY
    jd.jobdept
ORDER BY
    average_annual_salary_range DESC;

-- Q.3.Which job roles offer the highest salary?
SELECT
    jd.Job_ID,
    jd.name AS job_role,
    sb.amount AS salary
...
WHERE
    sb.amount = (
        SELECT MAX(amount) FROM SalaryBonus
    );

-- Q.4.Which departments have the highest total salary allocation?
SELECT
    jd.jobdept AS department,
    SUM(sb.amount + sb.bonus) AS total_salary_allocation_with_bonus
FROM
    JobDepartment jd
JOIN
    SalaryBonus sb ON jd.Job_ID = sb.Job_ID
GROUP BY
    jd.jobdept
ORDER BY
    total_salary_allocation_with_bonus DESC;

-- 3. QUALIFICATION AND SKILLS ANALYSIS

-- Q.1.How many employees have at least one qualification listed?
SELECT COUNT(DISTINCT Emp_ID) AS employees_with_qualification
FROM Qualification;

-- Q.2.Which positions require the most qualifications?
SELECT
    Position,
...
-- Q.3.What is the average number of qualifications per department?
SELECT
    jd.jobdept AS department,
    AVG(qualification_count) AS avg_qualifications_per_employee
FROM
    JobDepartment jd
JOIN
...
-- Q.1.Which year had the most employees taking leaves?
SELECT
...
-- Q.2. What is the average number of leave days taken by its employees per department?
WITH LeaveCounts AS (
    SELECT 
        emp_ID,
...
SELECT 
    jd.jobdept AS department,
    AVG(COALESCE(lc.total_leave_days, 0)) AS avg_leave_days_per_employee
FROM 
    JobDepartment jd
JOIN 
    Employee e ON jd.Job_ID = e.Job_ID
LEFT JOIN 
    LeaveCounts lc ON e.emp_ID = lc.emp_ID
GROUP BY 
    jd.jobdept
ORDER BY 
    avg_leave_days_per_employee DESC;

-- Q.3. Which employees have taken the most leaves?
SELECT 
    e.emp_ID,
    e.firstname,
    e.lastname,
...
-- Q.4.What is the total number of leave days taken company-wide?
SELECT 
    COUNT(*) AS total_leave_days
FROM 
    Leaves;

-- Q.5.How do leave days correlate with payroll amounts?
WITH LeaveCounts AS (
    SELECT 
        emp_ID,
        COUNT(*) AS total_leave_days
...
SELECT
    ...
    e.firstname,
    e.lastname,
    COALESCE(lc.total_leave_days, 0) AS total_leave_days,
    COALESCE(pt.total_payroll_amount, 0) AS total_payroll_amount
FROM
    Employee e
LEFT JOIN
    LeaveCounts lc ON e.emp_ID = lc.emp_ID
LEFT JOIN
    PayrollTotals pt ON e.emp_ID = pt.emp_ID
ORDER BY
    total_leave_days DESC;

-- 5. PAYROLL AND COMPENSATION ANALYSIS

-- Q.1.What is the total monthly payroll processed?
SELECT
    EXTRACT(YEAR FROM date) AS year,
...
-- Q.2.What is the average bonus given per department?
SELECT
    jd.jobdept AS department,
...
-- Q.3.Which department receives the highest total bonuses?
SELECT
    jd.jobdept AS department,
...
-- Q.4. What is the average net salary after all deductions?
SELECT
    AVG(total_amount) AS average_net_salary
FROM
    Payroll;

-- 6. EMPLOYEE PERFORMANCE AND GROWTH

-- Q.1.Which year had the highest number of employee promotions?
SELECT
